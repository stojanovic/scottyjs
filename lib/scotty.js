...
function scotty(options = {
  ...
  if (!options.source || !options.bucket || !options.region)
    return Promise.reject('Source, bucket and region are required')

  return createOrUpdateBucket(options.bucket, options.region, options.update, options.force, options.quiet, options.logger)
    .then(result => {
      if (!options.quiet)
