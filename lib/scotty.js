...
function scotty(source, bucket, region, website, spa, update, zoneId, dnsName, force, quiet, logger) {
  ...
  return createOrUpdateBucket(bucket, region, update, force, quiet, logger)
    ...
    .then( response => {
      return new Promise((resolve) => {
        if( response.domain && zoneId && dnsName ) {
          if (!quiet)
            logger.log('   config'.magenta, '✤', 'set r53 route '.cyan)
          return createRoute(response.domain, zoneId, dnsName)
            .then( change => {
              if (!quiet)
                logger.log('   config'.magenta, '✤', 'set r53 route II'.cyan)
