...
function vaporizeBucket(bucket, quiet, logger) {

  return deleteBucket(bucket)
    .then(location => {
      if (!quiet)
        logger.log('   vaporize'.magenta, '✤', colors.cyan(`${location} bucket`))

      return bucket
    })
    .then(response => {
      if (!quiet) {
        logger.log('\nSuccessfully vaporized!'.magenta, colors.cyan(bucket))
      }
      return response
    })
    .catch(err => {
...
function scotty(source, bucket, region, website, spa, update, vaporize, force, quiet, logger) {
  logger.log(vaporize)
  if (vaporize){
    ...
    return vaporizeBucket(bucket, quiet, logger).then()
      .catch(err => {
        if (!quiet)
