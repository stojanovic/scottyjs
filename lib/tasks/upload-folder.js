...
function logPercentage(filesInfo) {
  const limitedPercentage = parseInt(filesInfo.percentage / 3, 10)
  process.stdout.clearLine()
  process.stdout.cursorTo(0)

  if (filesInfo.totalNum === filesInfo.uploadedNum) {
    process.stdout.write(colors.inverse.green('    Upload files    ') + ' ' + colors.inverse.green(Array(limitedPercentage + 1).join(' ')) + colors.inverse.gray(Array(34 - limitedPercentage).join(' ')) + ` ${filesInfo.percentage}% ${filesInfo.uploadedNum}/${filesInfo.totalNum} files ` + colors.dim(`(${getFormattedSize(filesInfo.uploadedSize)}/${getFormattedSize(filesInfo.totalSize)})`))
    return process.stdout.write('\n')
  }

  process.stdout.write(colors.inverse.yellow('    Upload files    ') + ' ' + colors.inverse.green(Array(limitedPercentage + 1).join(' ')) + colors.inverse.gray(Array(34 - limitedPercentage).join(' ')) + ` ${filesInfo.percentage}% ${filesInfo.uploadedNum}/${filesInfo.totalNum} files ` + colors.dim(`(${getFormattedSize(filesInfo.uploadedSize)}/${getFormattedSize(filesInfo.totalSize)})`))
}
