...
function createOrReuseBucket(options, s3, logger) {
  ...
  return s3
    ...
    .then(() => {
      if (options.log)
        logger('     create bucket' + colors.magenta(' ✤ ')  + `"${options.bucket}" S3 bucket is created in ${options.region ? options.region : 'your default'} region`)
    ...
    .then(options => {
      if (options.log)
        logger('     select bucket' + colors.magenta(' ✤ ')  + `"${options.bucket}" S3 bucket is selected`)
